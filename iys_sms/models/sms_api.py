...
class VerimorSmsApi(SmsApiBase):
    ...
    def _send_sms_batch(self, messages, delivery_reports_url=False):
        ...
        for message in messages:
            ...
            verimor_messages = [
                {'msg': content, 'dest': rec['number'], 'id': rec['uuid']}
                for rec in numbers
            ]
            payload = {
            ...
            try:
                resp = requests.post(_VERIMOR_SMS_ENDPOINT, json=payload, timeout=20)
            ...
            if resp.status_code == 200:
                results.extend([{'uuid': rec['uuid'], 'state': 'success'} for rec in numbers])
            else:
                results.extend([
                    {'uuid': rec['uuid'], 'state': 'server_error', 'failure_reason': resp.text.strip()}
                    for rec in numbers
                ])
        return results
